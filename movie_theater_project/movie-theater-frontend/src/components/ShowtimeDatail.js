...
function ShowtimeDetail() {
  ...
  const fetchAllBookings = async (userId) => {
    ...
    let page = 1;
    while (true) {
      const res = await fetchBookingsByUser(userId, page); // Assumes fetchBookingsByUser accepts page
      const newBookings = res.data.results || res.data; // Handle paginated or direct response
    ...
    if (user) {
      fetchAllBookings(user.id)
        .then(bookings => {
          ...
          const userBookings = bookings.filter(booking => booking.showtime.id === showtimeId && booking.status === 'Confirmed');
